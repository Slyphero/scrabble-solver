...
public:
    Game(const Player &player, Gaddag *gaddag, const Board &board, 
         const Position &initialPosition) : currentState(player, gaddag, board, 
                                                         initialPosition, false) {}

    std::string buildMot(Board board, Direction direction, Position pos);

    bool isPossible(Board board, Direction direction, Position pos, Gaddag *gaddag);
     ...
     */
    std::pair<State, int> getBestPlayFromPosition(Position position, Direction direction,
                                                  Gaddag *root);

    std::pair<State, int> getBestPlayOnEmptyBoard();

    bool checkValidCollision(const Board &board, Gaddag *gaddag, Direction direction, 
    ...
    void applyBestPlay(std::pair<State, int> play, Gaddag *root, LettersCollection &bag);
