                                                    ...
                                                    Gaddag* root) 
{
    ...
    while (!states.empty()) 
    {
        ...
        bool hasPlayed = (startSize > (int)analyzedState.player.getInventory().size());
        ...
        {
            char letter = analyzedState.board(analyzedState.currentPosition.line,
                                              analyzedState.currentPosition.column).letter;
            if (letter == 0) 
            {
                if (analyzedState.currentGaddag->checkIfFinal() &&
                ...
                Position newPosition = analyzedState.currentPosition.findNextPosition(direction, 
                ...
                {
                    if (analyzedState.currentGaddag->getGaddagByLetter(tile.getLetter()) != nullptr) 
                    {
                        ...
                        board(analyzedState.currentPosition.line,
                              analyzedState.currentPosition.column).letter = tile.getLetter();

                        State newState(player,
                                       analyzedState.currentGaddag->getGaddagByLetter(tile.getLetter()),
                                       board, 
                                       newPosition,
                                       analyzedState.isPlusHasBeenFound);
                        ...
                        */
                                       ...
                                       analyzedState.currentPosition, root)) 
                        {
                            if (newState.board(newState.currentPosition.line,
                                            newState.currentPosition.column).letter == 0) 
                            {
                                states.push(newState);
                            } 
                            else if (checkValidCollision(newState.board,
                                                         newState.currentGaddag, 
                                                         direction,
...
int Game::scoreAll(Board board, Direction direction, Position pos) 
{
    ...
    int scorethis = letterCollection.getPoint(board(pos.line, pos.column).letter) *
                    board(pos.line, pos.column).bonus.letter_factor;
...
bool Game::isPossible(Board board, Direction direction, Position pos, Gaddag* gadd) 
{
...
std::pair<State, int> Game::getBestPlayOnEmptyBoard() 
{
    ...
    for (Direction direction : directions) 
    {
        ...
        while (!states.empty()) 
        {
            ...
            char cellLetter = state.board(state.currentPosition.line, state.currentPosition.column).letter;

            if (cellLetter == 0) 
            {
                for (const Letter& tile : state.player.getInventory()) 
                {
                    Gaddag* nextGaddag = state.currentGaddag->getGaddagByLetter(tile.getLetter());
                    ...
                    if (nextGaddag != nullptr) 
                    {
                        ...
                        nextBoard(state.currentPosition.line, state.currentPosition.column).letter = tile.getLetter();

                        Position nextPosition = state.currentPosition.findNextPosition(direction, 
                                                                                       state.isPlusHasBeenFound);

                        State nextState(nextPlayer, nextGaddag, nextBoard, 
                                        nextPosition, state.isPlusHasBeenFound);
...
bool Game::checkValidCollision(const Board& board, Gaddag* gaddag, Direction direction, 
                               bool isPlusHasBeenFound, Position position) 
{
    Position nextPosition = position.findNextPosition(direction, isPlusHasBeenFound);

    char nextLetter = board(nextPosition.line, nextPosition.column).letter;

    while (nextPosition.checkIfValid() && nextLetter != 0) 
    {
        if (gaddag->getGaddagByLetter(nextLetter) == nullptr) 
        {
            return false;
        }
...
std::pair<State, int> Game::getBestOverallPlay(Gaddag* root) 
{
    ...
    for (int line = 0; line < 15; line++) 
    {
        for (int column = 0; column < 15; column++) 
        {
            ...
            {
                std::pair<State, int> pair1 = getBestPlayFromPosition(position, HORIZONTAL, root);
                std::pair<State, int> pair2 = getBestPlayFromPosition(position, VERTICAL, root);
...
void Game::applyBestPlay(std::pair<State, int> play, Gaddag* root, LettersCollection& bag) 
{
